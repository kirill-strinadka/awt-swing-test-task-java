...
Актуально для исходников под `src/main/java`, `src/test/java` и корня модуля. Каталог `target/` после сборки не перечисляется.
...
└── src/
    ...
    │       ├── SessionUnregistrationCloseHandler.java
    │       ├── Sha256PasswordVerifier.java
    │       └── UuidMessageIdGenerator.java
    └── test/java/com/kstrinadka/chat/server/
        ├── infrastructure/
        │   ├── DefaultProtocolValidatorTest.java
        │   └── JacksonProtocolMessageCodecTest.java
        └── transport/
            └── ChatServerSmokeTest.java
```
